...
async def feature_engineer(request: Request):
    ...
    for col, t_list in (config.get("transforms") or {}).items():
        ...
        if "log1p" in t_list:
            name = f"{col}_log1p"
            df[name] = _np.log1p(_np.maximum(vals, 0))
            new_columns.append(name)

        if "sqrt" in t_list:
            name = f"{col}_sqrt"
            df[name] = _np.sqrt(_np.maximum(vals, 0))
            new_columns.append(name)
