...
| E17 | ☑ | No way to block an IP on the backends | Fixed (uncommitted). Vercel Firewall blocks IPs for ml-portfolio's `/api/*`, but browsers call the HF Spaces directly and HF has no firewall. Both backends now honour a `BLOCKED_IPS` Space variable (comma-separated single IPs and/or CIDR): ml-api `security/ip_block.py` (middleware added last so it runs first) and ml-sql `routers/_guard.py` (checked before every other guard), both keyed on X-Forwarded-For like the rate limiters. Unset = nothing blocked; editing it restarts the Space (~2 min), so a block is not instant. Verified against both real apps: a listed single IP and a CIDR member get 403, other IPs 200, invalid entries ignored, empty config blocks nothing. **Dormant until the user sets `BLOCKED_IPS`** on each Space. |
