...
| E10 | ☐ | **ml-sql: DuckDB uploads can read any server file, including the Space's API keys** | Found 2026-09-14 in E2. CSV/Parquet uploads open an unrestricted in-memory DuckDB; `/sql/page` and `/sql/filter` take SQL from the caller, and `validate_sql` blocks keywords, not DuckDB's file functions. One upload plus `SELECT * FROM read_csv('/proc/self/environ')` returns the environment. Proven locally with a planted secret file through the real `validate_sql` → `execute_duckdb` path; not tried live. The LLM path (`/sql/query`) can be steered to the same query. Fix: `enable_external_access=false` + `lock_configuration=true` after the upload is loaded. **Key rotation for the ml-sql Space: user, later.** |
| E11 | ☐ | ml-portfolio `/api/ai-tools`: caller controls cost on server keys | `model` and `maxTokens` come from the request unchecked, `messages`/`toolContext` have no size cap, and the default provider is Gemini — the billed key. |
| E12 | ☐ | ml-portfolio `/api/ai-explain`: public, unbounded, billed-first | Anyone can call it; it prefers the Claude then Gemini key and pastes caller-sent `stats` into the prompt with no size cap. Upstream error text is returned to the caller. |
| E13 | ☐ | ml-portfolio `/api/chat`: no cap on message count or length | Every request runs on server keys; only the WAF's 20/min per IP bounds it. |
| E14 | ☐ | ml-sql `/sql/connect`: server connects to any caller-given host | SSRF — the Space can be used to probe other hosts. Lower impact on HF, but unbounded. |
| E15 | ☐ | E2 remainder: ml-api backend not yet audited | RAG, document and assistant tools — prompt injection via uploaded content, cross-session data, output handling, cost bounds. |
...
7. Only section E remains. **E10 first (key leak in ml-sql), then E11–E14, then E15.** Key rotation for E9 and E10 is the user's, later.
