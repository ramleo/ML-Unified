...
| E3 | ☑ | Provider-failure alerting outside the document path | Done 2026-09-16 (ml-portfolio). The nightly evals catch a dead free provider on the document path (the paid key serving is itself the alarm); every other LLM route caught a provider 401/403 and moved on silently, so a stale free key could quietly escalate to the billed Claude/Gemini key with no signal. New `src/lib/providerAlert.ts`: `isAuthFailure()` distinguishes a dead/stale key (401/403, via the error's `.status` or an "invalid/expired/unauthorized" message) from transient 429/5xx/network failures, and `recordProviderAuthFailure()` writes an `auth_failure` row to the `llm_calls` table the analytics dashboard already reads, plus a greppable `PROVIDER_AUTH_FAIL` server-log tag. Gated by `analyticsWritesEnabled()` (no local pollution) and throttled to one row per (route, provider) per 10 min per instance so a dead key can't flood the table. Wired into `ai-explain` (whole cascade is server keys), `chat` (always server keys; its Claude/Groq throws now carry the status), and `ai-tools` (only when no user key — a user-supplied key failing is the caller's own problem). Chosen approach + scope were the user's call (dashboard log, not email; ml-portfolio's 3 routes, not the HF Spaces). Verified: tsc + eslint clean; `isAuthFailure` classifies 8 real auth-error shapes as auth and 6 transient/network shapes as not. **Not covered (by choice):** the HF Spaces' own cascades (ml-api, ml-sql) — a later pass if wanted. |
| E4 | ☑ | ml-sql's two untested provider paths | Done 2026-09-16. ml-sql had **no** test suite and was not in CI at all — the E10 and E14 guards were proven once by hand and had nothing watching them. New `services/ml-sql/tests/`: `test_duckdb_conn.py` (E10), `test_conn_guard.py` (E14, 25 cases, DNS mocked so it is hermetic), `test_providers.py` (the untested provider paths — each provider unwraps a different response shape, mocked HTTP, no key spent, plus 429→RateLimitError and the unknown-provider groq fallback). 39 tests. A new `test-ml-sql` job in `ci.yml` runs them on every push/PR (light deps, no torch, so its own job). Verified locally in a venv with duckdb+skops: 39 passed, ruff clean; the E10 test was shown to fail against the pre-fix broken open (plain connect leaks the planted secret via `read_text`/`read_csv`; the locked connection refuses with "file system operations are disabled by configuration"). |
...
| E18 | ☑ | **ml-api `/shap/custom/upload`: uploaded pickle = remote code execution** | Found + fixed 2026-09-14. The route did `joblib.load` on an uploaded file; joblib is pickle, which runs embedded code on load, so one `curl` of a crafted file ran arbitrary commands on the Space — every env key and the HF token. Proven locally: a crafted file loaded the exact way the route loads it ran a command (harmless marker). Origin enforcement does not stop a no-Origin `curl`, so it was reachable; public since the repo went public. Fixed in `a93a951` (Space `3bff11d`), live: new `routers/core/skops_safe.py` loads only **skops** files, and only when every type is under an ML-library allowlist (sklearn/numpy/scipy/xgboost/lightgbm) — a file naming `posix.system`, `builtins.eval` or any other type is refused before anything is built; `export_model` now emits `.skops` too so the train→export→re-upload round-trip stays intact. Verified: genuine skops loads and predicts identically; a joblib `.pkl`, an evil pickle (no code ran) and a skops file naming an unknown type are all refused. Verified live after rebuild: health 200, genuine skops → SHAP result, joblib `.pkl` → 400, evil pickle → 400 (no code ran). **Follow-up:** ml-portfolio `Step4Results.tsx` label fixed 2026-09-16 — the winner's "↓ Download .pkl" button hits `/models/{id}/export`, which returns `.skops`, so it now reads "↓ Download .skops"; the other button ("Download Model (.pkl)") hits `/model/{id}/download`, which genuinely returns a joblib `.pkl`, and is correct as-is. The file was 410 lines (over the gate), so the AI-analysis panel was extracted to `AutoMLAnalysisPanel.tsx` first (240 + 243 lines, both under 400); pure presentational split, tsc + eslint clean. **Still open:** ml-api's legacy `frontend/index.html` (5016 lines) still uploads `.pkl` — over the gate, left for a split; harmless because the skops-only endpoint rejects its `.pkl`. Regression test added 2026-09-16 — `services/ml-api/tests/test_skops_safe.py` (already in CI's ml-api job): a genuine model round-trips and predicts identically; a joblib `.pkl`, an evil pickle (marker file proves its payload never ran) and a skops file naming a non-allowlisted type are all refused. 5 tests, verified locally in a skops venv. |
| E19 | ☑ | ml-api automl `/explain` `custom` provider: no https/public-host check on `custom_base_url` | Fixed (uncommitted). The route sent the outbound request to any caller-given URL (SSRF-lite; only the caller's own key, never a server key). `inference.py` now requires `custom_base_url` to be `https` and to resolve to a public host (via `security_shared.resolve_public_ip`, the scanner tools' helper), else 400 (`0864438`, Space `66cd7fc`, live). Verified locally: http, localhost, 127.0.0.1, 169.254.169.254, 10/8, `::1` and `file://` refused; api.openai.com and other public hosts allowed. |
| E20 | ☑ | ml-api `attack_surface_check` CMS fingerprint followed redirects by hostname | Fixed (uncommitted). `_check_cms_fingerprint` used `follow_redirects=True`, so a host that passed `resolve_public_ip` could redirect the scanner to an internal address that check never saw. Now `follow_redirects=False` (`9ed10b1`, Space `52cce12`, live), matching the file's other two checks; the generator meta tag is on the landing page, so nothing legitimate is lost. Residual (accepted): a DNS-rebinding window between resolve and connect-by-hostname remains for all three checks — closing it fully would break SNI virtual hosts, not worth it for a tool whose only output is a version string. |
...
| E17 | ☑ | No way to block an IP on the backends | Fixed in `7dcba14` (Spaces `559d6de` ml-unified, `311ad0f` ml-sql), live. Vercel Firewall blocks IPs for ml-portfolio's `/api/*`, but browsers call the HF Spaces directly and HF has no firewall. Both backends now honour a `BLOCKED_IPS` Space variable (comma-separated single IPs and/or CIDR): ml-api `security/ip_block.py` (middleware added last so it runs first) and ml-sql `routers/_guard.py` (checked before every other guard), both keyed on X-Forwarded-For like the rate limiters. Unset = nothing blocked; editing it restarts the Space (~2 min), so a block is not instant. Verified against both real apps: a listed single IP and a CIDR member get 403, other IPs 200, invalid entries ignored, empty config blocks nothing. Both Spaces came back healthy on the new code (RUNNING after app-start = the new middleware imported cleanly); ml-sql guard confirmed still active. **Dormant until the user sets `BLOCKED_IPS`** on each Space. |
| E21 | ☑ | ml-api model routes accept unauthenticated no-Origin requests | Found 2026-09-16 while cleaning up test models; fixed the same day (partial, by design). `DELETE /models/{id}` deletes any non-built-in model with no auth beyond the origin check, and that check deliberately lets no-Origin requests through, so a plain `curl -X DELETE https://…/models/<id>` succeeded (proven — deleted 8 leftover test models this way). Fix: `security/origin_policy.origin_present_and_allowed()` + a guard on `delete_model` (now takes `Request`) that requires an allowlisted Origin, refusing a no-Origin or foreign-Origin delete with 403 before any deletion; built-ins stay protected, and the legacy Space UI's same-origin delete still works (the Space's own origin is allowlisted when `SPACE_HOST`/`SPACE_ID` is set). Regression test `tests/test_model_delete_origin.py` (5 cases: no-Origin→403, foreign→403, allowed+unknown→404, built-in+allowed→400, built-in+no-Origin→403). **Deliberately partial:** a curl can forge the Origin header, so this stops the accidental/naive case, not a determined script — Turnstile/auth would, judged disproportionate for a route whose worst case is deleting a regenerable demo model. `/train` left as-is: it is already behind per-IP rate limits, and origin-requiring it would be equally forgeable while breaking 4 CI tests. Decision (dashboard-log depth, DELETE-only) was the user's, 2026-09-16. |
...
7. Only section E remains. E10–E20 all fixed; the sweep's guard proofs (E10,
   E14, E18) are now permanent regression tests, and E4 closed with them by
   putting ml-sql in CI for the first time. Still open: **E3** (provider-failure
   alerting), **E5** (two files near the 400-line gate — split before adding),
   **E6** (handbook clips at 25 of 50). Key rotation for E9 and E10 is the
   user's, later.
