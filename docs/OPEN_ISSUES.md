...
| E21 | ☐ | ml-api model routes accept unauthenticated no-Origin requests | Noted 2026-09-16 while cleaning up test models. `DELETE /models/{id}` deletes any non-built-in model, and the automl train route creates one, with no auth beyond the origin check — and the origin check deliberately lets no-Origin requests through (curl, healthchecks, server-to-server), so a plain `curl -X DELETE https://…/models/<id>` succeeds (built-ins `titanic/iris/diabetes/insurance` are refused). Proven incidentally: deleted 8 leftover test models this way (HTTP 200 each). **Low severity** — no key leak, and the worst case is deleting or spamming regenerable demo models (model ids are the public `slugify(name)`, so a stranger would have to guess names); per-IP rate limits + the 10 MB body cap already bound abuse. **Fix if addressed:** a shared-secret/admin header on the mutating model routes (`DELETE /models/{id}`, train, `/model/{id}/download`), or restrict them to authenticated origins only. Not urgent. |
